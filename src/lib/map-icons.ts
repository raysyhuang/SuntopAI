...
// Direct center icon (hospital cross, solid teal)
export const directCenterIcon = new L.DivIcon({
  html: `
    <div class="relative flex items-center justify-center w-8 h-8">
      <div class="absolute inset-0 rounded-full bg-teal-500 border-2 border-white shadow-lg"></div>
...
// Partner center icon (handshake, outlined purple)
export const partnerCenterIcon = new L.DivIcon({
  html: `
    <div class="relative flex items-center justify-center w-7 h-7">
      <div class="absolute inset-0 rounded-full border-2 border-dashed border-purple-500 bg-slate-900/80"></div>
      <svg class="relative w-3.5 h-3.5 text-purple-400 z-10" fill="currentColor" viewBox="0 0 24 24">
...
export const createClusterIcon = (cluster: any) => {
  ...
  return new L.DivIcon({
    html: `
      <div class="marker-cluster marker-cluster-${size} ${sizeMap[size]} flex items-center justify-center rounded-full bg-teal-500/20 border-2 border-teal-500/80 text-teal-300 font-semibold shadow-lg">
        ${count}
      </div>
    `,
